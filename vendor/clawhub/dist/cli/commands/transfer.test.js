...
const { cmdTransferAccept, cmdTransferCancel, cmdTransferList, cmdTransferReject, cmdTransferRequest, } = await import('./transfer');
function makeOpts() {
    return {
        workdir: '/work',
        dir: '/work/skills',
...
describe('transfer commands', () => {
    ...
    it('request calls transfer endpoint', async () => {
        ...
        const requestArgs = mockApiRequest.mock.calls[0]?.[1];
    ...
    it('accept/reject/cancel call action endpoints', async () => {
        ...
        expect(mockApiRequest).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ method: 'POST', path: '/api/v1/skills/demo/transfer/accept' }), expect.anything());
        expect(mockApiRequest).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ method: 'POST', path: '/api/v1/skills/demo/transfer/reject' }), expect.anything());
        expect(mockApiRequest).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ method: 'POST', path: '/api/v1/skills/demo/transfer/cancel' }), expect.anything());
    });
});
