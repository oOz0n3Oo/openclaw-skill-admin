...
async function loadHttpModuleWithBunMocks(opts) {
    ...
    bunRuntimeMocks.spawnSync.mockImplementation((...args) => spawnSync(...args));
    ...
    bunRuntimeMocks.readFile.mockImplementation(async () => (opts?.readFileValue ?? Buffer.from([1, 2, 3])));
    return {
        http,
        spawnSync: bunRuntimeMocks.spawnSync,
        mkdtemp: bunRuntimeMocks.mkdtemp,
...
describe('http bun runtime', () => {
    ...
    it('uses curl for apiRequest GET and parses JSON', async () => {
        ...
        const result = await httpClient.apiRequest('https://registry.example', {
            method: 'GET',
            path: '/v1/ping',
            token: 'clh_token',
        });
    ...
    it('retries bun apiRequest on 429 errors', async () => {
        ...
        const { http: httpClient } = await loadHttpModuleWithBunMocks({ spawnImpl: spawnSync });
        await expect(httpClient.apiRequest('https://registry.example', {
            method: 'GET',
            path: '/v1/ping',
    ...
    it('supports fetchText bun path and propagates status fallback', async () => {
        ...
        await expect(httpClient.fetchText('https://registry.example', { path: '/v1/readme' })).rejects.toThrow('HTTP 400');
    });
    it('handles downloadZip bun path and cleans up temp dir', async () => {
        ...
        await expect(httpClient.downloadZip('https://registry.example', { slug: 'demo', token: 't' })).rejects.toThrow('not found');
    ...
    it('posts multipart form via curl in bun path', async () => {
        ...
        const result = await httpClient.apiRequestForm('https://registry.example', {
            method: 'POST',
            path: '/upload',
            form,
        });
