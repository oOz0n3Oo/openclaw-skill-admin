...
import { apiRequest, apiRequestForm, downloadZip, fetchText, registryUrl, shouldUseProxyFromEnv, } from './http';
import { ApiV1WhoamiResponseSchema } from './schema/index.js';
function mockImmediateTimeouts() {
    const setTimeoutMock = vi.fn((callback, _ms) => {
        callback();
        return 1;
    });
...
describe('shouldUseProxyFromEnv', () => {
    ...
    it('ignores NO_PROXY-only configs', () => {
        ...
        expect(shouldUseProxyFromEnv({})).toBe(false);
    });
});
describe('registryUrl', () => {
    it('works with a plain-origin registry (no base path)', () => {
        expect(registryUrl('/api/v1/skills', 'https://clawhub.ai').toString()).toBe('https://clawhub.ai/api/v1/skills');
    });
    it('preserves the registry base path', () => {
        const base = 'http://localhost:8081/custom/registry/path';
        expect(registryUrl('/api/v1/skills', base).toString()).toBe('http://localhost:8081/custom/registry/path/api/v1/skills');
    });
    ...
    it('handles compound paths with encoded segments', () => {
        ...
        expect(registryUrl(path, base).toString()).toBe('http://localhost:8081/base/api/v1/skills/my-skill/versions');
    });
});
describe('apiRequest', () => {
    ...
    it('includes rate-limit guidance from headers on 429', async () => {
        ...
        vi.stubGlobal('fetch', fetchMock);
        await expect(apiRequest('https://example.com', { method: 'GET', path: '/x' })).rejects.toThrow(/retry in 34s.*remaining: 0\/20.*reset in 34s/i);
    ...
    it('interprets legacy epoch Retry-After values as reset delays', async () => {
        ...
        vi.stubGlobal('fetch', fetchMock);
        await expect(apiRequest('https://example.com', { method: 'GET', path: '/x' })).rejects.toThrow(/retry in 40s.*remaining: 0\/20/i);
        vi.restoreAllMocks();
        vi.unstubAllGlobals();
    });
...
describe('apiRequestForm', () => {
    ...
    it('uses the longer upload timeout for multipart requests', async () => {
        ...
        expect(caught).toBeInstanceOf(Error);
        expect(caught.message).toMatch(/timed out after 120s/i);
...
describe('fetchWithTimeout — non-Error normalization', () => {
    it('wraps DOMException-like non-Error throws into proper Error instances', async () => {
        ...
        expect(caught).toBeInstanceOf(Error);
        expect(caught.message).toContain('The operation was aborted');
        vi.unstubAllGlobals();
    });
});
